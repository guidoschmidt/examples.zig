...
fn write_header(seconds: u32, file: std.fs.File.Writer) !void {
    ...
    try file.writeInt(u16, @truncate(CHANNELS * (BIT_DEPTH / BYTE_SIZE)), .little);
    try file.writeInt(u16, @truncate(BIT_DEPTH), .little);
    try file.writeAll("data");
    try file.writeInt(u32, num_samples * CHANNELS * (BIT_DEPTH / BYTE_SIZE), .little);
}

fn sine_wave(seconds: u32, file: std.fs.File.Writer, freq: f64) !void {
    var idx: u32 = 0;
