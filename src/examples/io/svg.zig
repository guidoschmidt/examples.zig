...
pub fn main() !void {
    const svg = f.partialGeneric([]const u8, wrap, .{
        allocator,
        "svg",
        "viewBox=\"0 0 800 300\" xmlns=\"http://www.w3.org/2000/svg\"",
    });
    const g = f.partialGeneric([]const u8, wrapNoOpts, .{ allocator, "g" });
    const t = f.partialGeneric([]const u8, wrap, .{
        allocator,
        "text",
        "fill=\"black\" x=\"0\" y=\"0\"",
    });
